...
class AgentConfigLoader {
   ...
   * @param {Object} options - Load options
   * @param {boolean} options.skipCache - Skip cache and force reload
   * @param {string} options.squad - Optional squad name to look in
  ...
  async loadAgentDefinition(options = {}) {
    ...
    let agentPath;
    if (options.squad) {
      agentPath = path.join(process.cwd(), 'squads', options.squad, 'agents', `${this.agentId}.md`);
    } else {
      // Try core first, then search in squads if not found
      const corePath = path.join(process.cwd(), '.aiox-core', 'development', 'agents', `${this.agentId}.md`);
      try {
        await fs.access(corePath);
        agentPath = corePath;
      } catch {
        // Search in squads
        const squadsPath = path.join(process.cwd(), 'squads');
        try {
          const squads = await fs.readdir(squadsPath);
          for (const squad of squads) {
            const potentialPath = path.join(squadsPath, squad, 'agents', `${this.agentId}.md`);
            try {
          ...
          // No squads directory or error reading it
        }
      }
    }

    if (!agentPath) {
      throw new Error(`Agent file not found for ID: ${this.agentId}`);
    }
    ...
    try {
      ...
      if (contentTrimmed.startsWith('---')) {
        const nextSeparator = contentTrimmed.indexOf('---', 3);
        if (nextSeparator !== -1) {
          yamlContent = contentTrimmed.substring(3, nextSeparator).trim();
        }
      }

      // Try YAML code block if frontmatter not found
      if (!yamlContent) {
        const blockStart = content.indexOf('```yaml');
        const blockStartAlt = content.indexOf('```yml');
        const finalStart = blockStart !== -1 ? blockStart + 7 : (blockStartAlt !== -1 ? blockStartAlt + 6 : -1);

        if (finalStart !== -1) {
          const blockEnd = content.indexOf('```', finalStart);
          if (blockEnd !== -1) {
            yamlContent = content.substring(finalStart, blockEnd).trim();
          }
        }
      }

      if (!yamlContent) {
        throw new Error(`No YAML block or Frontmatter found in ${this.agentId}.md`);
      }
      
      let agentDef;
      try {
        agentDef = yaml.parse(yamlContent);
      } catch (parseError) {
        // Try normalizing compact command format before parsing
        const normalizedYaml = this._normalizeCompactCommands(yamlContent);
        try {
